...
import { DiskResponse, DiskResponseSchema } from './diskTypes';

/**
 * Fetch the disk-resilience gauge feed (Hearth — MONITOR pillar).
 *
 * Polled authed REST GET (the apiVram pattern), NOT socket. Reads the disk-sentinel
 * feed via the server projection. Defensive: on a non-OK status or unparseable body,
